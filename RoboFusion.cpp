...
const ll INF=1e18;
const int mod=1e5+7;
int dfs(int i,int j,vector<pair<int,int>>&arr,vector<vector<int>>&dp){
	if(i>=j){return 0;}
	if(dp[i][j]!=-1){return dp[i][j];}
	int ans=INT_MAX;
	for(int ii=i;ii<j-1;ii++){
		int val=dfs(i,ii,arr,dp)+dfs(ii+1,j,arr,dp)+arr[i].first*arr[ii].second*arr[j].second;
		ans=min(ans,val);
	}
	return dp[i][j]=ans;
}
